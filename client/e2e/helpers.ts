...
/** Creates a new TwoPlayer game from the landing screen and returns its id (parsed from the URL). */
...
/**
 * Creates a new VsAi game (human plays White against the server's heuristic
 * AI) from the landing screen and returns its id (parsed from the URL). The
 * backend starts these games in the Active status with a synthetic Black
 * seat, so — unlike a TwoPlayer game — there is no waiting-for-opponent
 * phase and no join link to share.
 */
